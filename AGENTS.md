...
колонке `success` нечитаема. Уже собранные 36 эпизодов `ru_case_swap` не
переписывались: они помечены устаревшими в `data/rollout_provenance.json`, в
метриках 500 эпизодов из 536. Пересчитать их офлайн из сохранённых
`object_poses` возможно — это открытое решение пользователя.
