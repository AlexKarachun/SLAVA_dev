...
**D5: пайплайн исправлен, GreenVLA пересняты.** Найдены и устранены две
ошибки на стороне наблюдений (см. `slava-greenvla`): проприоцепция подавалась
в мировой системе координат вместо системы базы робота, и слот гриппера был
инвертирован. Актуальное покрытие `rollouts/rollout_annotations.jsonl`:

```
GreenVLA-R0            0/27    полное покрытие (последний эпизод не досчитан)
GreenVLA-R1 (bridge)   5/28    полное покрытие  <- было 0/28 до фиксов
GreenVLA-R2 (bridge)   2/28    полное покрытие
OpenVLA-OFT           74/99    полное покрытие
pi0                    2/127   исключены из метрик (см. ниже)
pi0.5                  0/127   исключены из метрик
SmolVLA                1/114   исключены из метрик, покрытие неполное
```
...
2. **Коммиты и пуш** — только по явной просьбе. Исключение — конец
   сессии/handoff (skill `slava-session-handoff`), где коммит+пуш всего
   нужного для переноса на другую машину входит в процесс по умолчанию.
3. **Досбор SmolVLA** (114/127) и решение, нужен ли он вообще до того, как
   найдена причина низкого SR.
...
- **Внутри одной GPU, если это уместно по объёму задачи, старайтесь выжимать
  ...
  прецеденты в этом файле: SmolVLA 2.4GB безопасно уживается вторым процессом
  на одной GPU, а OpenVLA-OFT 7B — нет, упирается в OOM; всегда проверяйте
  реальный объём VRAM конкретного чекпойнта, а не предполагайте по аналогии).
