...
**Зацепка, которую стоит проверить следующей** (найдена, но ещё не
проверена экспериментом): эталонная обёртка SimplerEnv для
`widowx_bridge` (`simpler_env/policies/octo/octo_model.py`)
**бинаризует** гриппер — `action["gripper"] = 2.0 * (open_gripper > 0.5)
- 1.0`, — тогда как наш `greenvla_server.py` пропускает непрерывное
значение через `2*x - 1`. Это тот же класс бага, что уже был найден у
OpenVLA-OFT (там отсутствовал `normalize_gripper_action(binarize=True)`):
непрерывное значение 0.6 даёт слабое +0.2 вместо уверенного +1.0, то есть
гриппер физически недосмыкается. Проверять на GreenVLA (все три стадии) и
на lerobot-политиках в SimplerEnv.
