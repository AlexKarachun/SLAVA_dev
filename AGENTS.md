...
под квоты, как в пилоте отбирали 20 из 117. И запас тут скромнее, чем кажется —
в пилоте годными оказались 35% кандидатов (41 из 117); при той же доле 412
кандидатов дают ~144 годных при цели 120–180, то есть впритык, а квоты
(spatial 8/20, distractors, применимость `ru_case_swap`) сузят ещё. **Проверьте
запас до начала авторинга, а не после**; если не хватает — добирать init state
у уже собранных определений дешевле всего.
