...
Следующий этап — первые model rollouts (см. конец этого раздела), ещё не
начат. Порядок этапов — в "Порядок построения benchmark" ниже.
...
**Обязательный процесс для сессии, где это будет реализовываться:** сначала
объяснить пользователю своими словами, что конкретно будет сделано и как
(архитектура rollout-кода, как каждая из 5 моделей интегрируется с
LIBERO/SimplerEnv, схема логирования), дождаться его согласия — не начинать
писать код молча. По ходу реализации советоваться по каждому из открытых
вопросов выше, а не решать их самостоятельно явочным порядком.
