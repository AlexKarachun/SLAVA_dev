...
содержит то, что этап D4 передаёт дальше: `frames_v0.jsonl` (20 сцен, схема
v0.2, все поля реально заполнены, включая `mt_russian` и `token_len`),
`frames_v0.schema.json`, `prompts_v0.jsonl` (127 строк = 20 сцен × 7
primary-вариантов минус `axis_na`). QA-чеклист `task.md` (16 пунктов)
пройден полностью; native check закрыт (пользователь лично просмотрел RU
переформулировки и подтвердил LLM-draft оценки как human-verified).
Полный ручной+программный аудит всех 20 фреймов пройден.

**D5 (model rollouts) — инфраструктура готова, данные собраны частично.**
Архитектура (env-worker + model-server + оркестратор), авторазметка по
контракту `task.md`, отчёт, шардинг по GPU — всё написано и работает.
Фактическое покрытие `rollouts/rollout_annotations.jsonl` на момент записи
(успехов/эпизодов, ожидаемое покрытие — 127 на модель для pi0/pi0.5/SmolVLA,
99 для OpenVLA-OFT, 28 для GreenVLA):

```
GreenVLA-R0            0/28    полное покрытие
GreenVLA-R1 (bridge)   0/28    полное покрытие
GreenVLA-R2 (bridge)   6/28    полное покрытие  <- единственный ненулевой SR
OpenVLA-OFT           74/99    полное покрытие
pi0                    2/127   полное покрытие   * исключены из метрик, см. ниже
pi0.5                  0/127   полное покрытие   * исключены из метрик, см. ниже
SmolVLA                1/114   НЕПОЛНОЕ (114/127) * исключены из метрик, см. ниже
```

**Важно про отчёт:** `annotate_provenance()` в
`generate_rollout_report.py` сравнивает mtime первого кадра каждого эпизода
с mtime соответствующего model-server-файла и исключает из всех агрегатов
модели, чьи эпизоды пересекают изменение инференс-кода (не пулить две
разные конфигурации в одно число). Сейчас под это попадают **все три
lerobot-политики** (pi0/pi0.5/SmolVLA): они делят один
`lerobot_server.py`, и его последняя правка (device-фикс, нужный только
pi0.5) сделала «устаревшими» и уже собранные эпизоды pi0/SmolVLA. Проверка
намеренно консервативная — по файлу, а не по тому, затронула ли правка
конкретную модель. Итог: в метриках отчёта сейчас участвуют 183 эпизода из
551 (GreenVLA R0/R1/R2 + OpenVLA-OFT). **Чтобы получить валидные числа по
pi0/pi0.5/SmolVLA, их нужно переснять целиком после того, как
`lerobot_server.py` стабилизируется** — это, а не досбор недостающих 13
эпизодов SmolVLA, настоящий объём оставшейся работы по этим трём.

### Главная открытая проблема: подозрительно низкий SR

Кроме OpenVLA-OFT (74/99 — здоровый результат, совпадает с заявленным
авторами ~97% с поправкой на наши варианты инструкций) и GreenVLA-R2
(6/28 ≈ 21%), **все остальные модели дают SR близкий к нулю, что заметно
ниже заявленных авторами чисел**. Это главный открытый вопрос проекта.

Что уже проверено и исправлено (детали и доказательства — в skills, не
переоткрывать без новой причины): camera-swap для lerobot-политик на
LIBERO, rotation representation (Euler→axis-angle) у GreenVLA,
action-truncation (32→7 dim) у `pi0_base`/`pi05_base`, device-mismatch у
pi0.5, gripper post-processing у OpenVLA-OFT, `conda run` не форвардит
сигналы, `num_steps_wait=10`.

Что установлено про GreenVLA-R0 конкретно (см. `slava-greenvla`):
конфиг R0 объявляет `normalization_mode="quantile"` (у R1/R2 —
`"mean_std"`), и у quantile-формулы денормализации асимметричная нулевая
точка — сырой выход модели 0 не соответствует нулевому физическому
действию, что даёт систематический дрейф манипулятора вверх (~+0.45 м за
60 шагов, подтверждено численно на реальных замерах сырого выхода).
Принудительный `mean_std` убирает дрейф, но **не восстанавливает успех**
(0/48 на разных выборках), поэтому официальные результаты R0 считаются по
конфигу авторов, без override. R0 даёт 0/10 и на `eggplant`, и на
`carrot` — при заявленных авторами 88.5% и 25% success соответственно;
ноль на самой лёгкой задаче — сильный аргумент в пользу того, что
остаётся ещё не найденный системный баг, а не «модель просто слабая».

**Найден и исправлен крупнейший баг всей SimplerEnv-ветки (06.08.2026):
проприоцепция подавалась в мировой системе координат вместо системы
координат базы робота.** `norm_stats` самих чекпойнтов задают диапазоны
x∈[0.171,0.453], y∈[-0.169,0.236], z∈[-0.056,0.195] — это однозначно
база робота, а `link.get_pose()` в SAPIEN возвращает мировую позу, причём
база WidowX стоит на z≈0.87. Фактически модель на каждом шаге получала
нормализованные `[-3.24, 0.00, 7.46]` вместо ожидаемых ~[-1,1], у оси x
вообще противоположный знак всему обучающему диапазону. Затронуты все три
стадии GreenVLA и все lerobot-политики на SimplerEnv. Рядом, в том же
векторе состояния, найдена инверсия слота гриппера (модель обучена на
openness, подавали closedness). Оба исправлены в
`env_worker_simpler.py`/`greenvla_server.py`/`lerobot_server.py` и в
pure-upstream репро-скрипте.

**Важно про доверие к прошлым выводам:** прошлый «pure-upstream репро»,
который считался независимым подтверждением корректности пайплайна,
независимым не был — его `build_state()` писался руками по той же прозе и
содержал ровно те же две ошибки. Его результат (2/20) подтверждал общий
дефект, а не корректность. Все выводы, опиравшиеся на это совпадение
(«баг не наш, остался домен-гэп»), недействительны.

**Что при этом ещё НЕ доказано:** насколько фикс поднимает SR. Абляция на
n=10 на момент записи не разделяла варианты (контроль ~1/6 против ~2/7 с
фиксами) — это слишком малая выборка. Плюс контрольная ветка вообще дала
ненулевой результат там, где продакшн-прогон R1 дал 0/28: диагностика
ресетит через `env.reset(seed=k)`, а продакшн — через
`options={"obj_init_options": {"episode_id": ...}}` по 4 фиксированным
episode_id. Это разные раскладки объектов, сравнивать их числа напрямую
нельзя. Детали и метод проверки — в `slava-greenvla` и в разделе
«Debugging low SR» skill'а `slava-model-rollouts`.

**Отдельная зацепка, пока не проверенная:** эталонная обёртка SimplerEnv
(`simpler_env/policies/octo/octo_model.py`, ветка `widowx_bridge`)
**бинаризует** команду гриппера — `2.0 * (open_gripper > 0.5) - 1.0`, — а
мы пропускаем непрерывное значение. По замерам реальных эпизодов доля
команд в «нерешительной» середине: R0 97%, R1 0%, R2 7% — то есть для R0
это может быть существенно, для R1/R2 почти нет.
