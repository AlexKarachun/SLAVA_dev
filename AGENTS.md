...
эпизоды лежат в `rollouts/episodes_archived_pre_reset_fix/` — не удалены,
потому что это единственная запись о том, что делали скомпрометированные
прогоны. Логи серверов того пересбора — `rollouts/logs_rerun_3090/`.
...
pi0            4/20 = 20%  [ 8;42]   опубликованного числа нет
pi0.5          0/20 =  0%  [ 0;16]   опубликованного числа нет
GreenVLA-R0/R1/R2  n=4 каждая        не проверяемо (CI шире 40 п.п.)
```

Два открытых вопроса, которые надо закрыть до публикации любых цифр.
**Первый: pi0.5 даёт ноль успехов на всех 127 эпизодах** — ни на одной среде,
ни в одном варианте. При этом веса грузятся чисто, а метки показывают
реальное взаимодействие со сценой (тянется, промахивается, задевает не тот
объект). Ноль на 127 — подпись чего-то системного, а не слабой политики.
**Второй: у pi0 и pi0.5 в `published_baselines.json` стоит `null`** — авторского
SR под нашу связку мы не нашли, поэтому про эти две модели нельзя сказать
даже «совпало» или «нет». Дыру в проверке надо закрыть.
