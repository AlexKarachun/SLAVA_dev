...
**Пул кандидатов собран 08.08.2026 — заново не собирать.** `data/full_set/`
содержит **896 сцен**: 800 LIBERO (сьюты spatial/object/goal/10, 40 задач × 20
init state) и 96 SimplerEnv (4 bridge-задачи × 24 эпизода). 1696 кадров,
5312 объектов, усадка 40 шагов везде. Сцены `libero_90` отложены в
`data/full_set_archive_libero90/` — на них не дообучен ни один чекпойнт.

| Что | Состояние |
| --- | --- |
| `libero_inventory.jsonl` / `simpler_inventory.jsonl` | сырые выгрузки сборщиков |
| `task_inventory.jsonl` | объединённый инвентарь, 896 сцен — работать с ним |
| `visibility_review.html` | дашборд разметки видимости, готов к открытию |
| `images/libero`, `images/simpler` | 1696 кадров, битых путей нет |
| `data/libero_bddl/` | BDDL всех 40 задач LIBERO, включая `libero_10` |
| `collection_provenance.json` | пины репозиториев, версии, правило сэмплирования, счётчики |

Размер выбран под квоты: связывающая — пространственная (только 12 задач из 40
используют пространственный язык), и 20 init state дают ровно **x4** запаса к
цели в 150 отобранных сцен. Остальные квоты x11–x21. Разбор — `docs/FULL_SET_PLAN.md`.

**Что уже заполнено:** `success_predicates` у всех 896, объекты с `sim_handle`
и позами, метаданные воспроизводимости. **Что пусто и составляет работу:**
`usable_for_slava`, видимость (5312 объектов), `quota_eligibility`,
`candidate_slots` (кроме bridge), весь авторинг инструкций.

**У SimplerEnv нет BDDL и не нужен** — сцена там воспроизводится по
`gym_env_name` + `episode_id` + `reset_seed` + пину коммита, всё это в записи.
...
0. **Бюджет ручной проверки на полном наборе — решение пользователя, и его
   ...
   сценах и 5312 объектах это уже расписание, а не деталь. Варианты:
...
- `slava-remote-scene-collection` — **сбор сцен на арендованном сервере под
  таймером**: минимальная установка (CPU-torch, micromamba, обязательный
  `configure_libero.py`), `MUJOCO_GL=egl` вместо osmesa, разбиение по
  «сьют × задача», почему `load average` врёт при выборе параллельности, и что
  обязательно забрать с машины до её смерти (BDDL!). Не про роллауты.
