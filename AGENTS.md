...
**Сцены уже собраны — заново не собирать.** `data/full_set/` содержит
**412 сцен**: 390 LIBERO (все пять сьютов, 130 уникальных задач × 3 init
state) и 22 SimplerEnv (4 bridge-задачи). Отрендерены 802 кадра (50 МБ).
Инвентарные цели task.md это перекрывает: 130 задач уже в верхней половине
вилки 120–180.
