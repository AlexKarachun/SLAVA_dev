...
**D5 закрыт по сбору: 536 эпизодов, покрытие полное, все 536 идут в метрики.**
36 эпизодов `ru_case_swap` были помечены устаревшими 07.08.2026 после смены
критерия успеха, а 08.08.2026 пересчитаны офлайн из сохранённых `object_poses`
и возвращены (`scripts/relabel_rollouts.py --include-case-swap`).
...
GreenVLA-R0    0/22 =  0%  [ 0;15]   не опубликовано  СРАВНИТЬ НЕ С ЧЕМ
...
`ru_case_swap` — это зонд, а не задача: инструкция перевёрнута, а предикат среды
намеренно оставлен прежним (перевернуть его значило бы получить другую задачу с
другой физикой). **Критерий успеха для этого варианта изменён 07.08.2026 по
решению пользователя.** Раньше успех брался из предиката среды, и потому
`success=True` означало «модель НЕ заметила перестановку ролей и выполнила
исходное задание» — величина, обратная тому, что зонд измеряет. Теперь успех
считается из финальных поз: выполнил ли робот именно перевёрнутую инструкцию
(`auto_label._swapped_success`). Чем посчитан успех, видно в новом поле строки
аннотации `success_source` (`env` / `swapped_predicate`) — без него единица в
колонке `success` нечитаема. Уже собранные 36 эпизодов `ru_case_swap` сначала
были помечены устаревшими, а 08.08.2026 по решению пользователя пересчитаны
офлайн из сохранённых `object_poses` (`scripts/relabel_rollouts.py
--include-case-swap`) и возвращены в метрики — GPU для этого не потребовался.
В метриках все 536 эпизодов. **Результат зонда: OpenVLA-OFT 0/4** — во всех
четырёх сценах она выполнила исходную задачу и перестановку ролей не заметила.

**Там же изменено правило `negation_error`** (07.08.2026, найдено пользователем
при ручной валидации): метка ставится только на варианте `ru_negation`, потому
что запрет существует лишь там, где инструкция его произносит. Раньше она
срабатывала на любом варианте сцены, где рука задевала «запрещённый» объект: из
21 такой метки в пилоте только 4 приходились на ось отрицания, остальные 17
перемечены в `target_grounding_error`. Сырое поле `forbidden_object_touched`
осталось для всех вариантов.
...
**Решение 08.08.2026: в метриках отчёта (разделы 5 и 6) остаётся только
`primary`.** У `preliminary` английская база 5/22 — приводить по ней Δlang
значило бы выдавать за языковой эффект следствие неразобранного расхождения.
Все семь моделей по-прежнему видны в таблице достоверности и в галерее.

Отдельно про GreenVLA-R0: она `excluded`, но **не как «не воспроизводит»** —
опубликованного SR для этого чекпойнта не существует вовсе (проверено по
четырём источникам, `docs/EVAL_PARITY.md`). Не тратить GPU на проверку её
воспроизводимости на `en_canonical`: сравнивать не с чем.

### Полномасштабный авторинг: состояние на 08.08.2026

Полный разбор требований task.md к этой фазе, по шагам и с пометками, где
кончается контракт и начинаются наши решения, — [`docs/FULL_SET_PLAN.md`](docs/FULL_SET_PLAN.md).
Ниже — состояние данных и мины; план работ там.


**Незакрытый вопрос до начала работы: мы пропускаем ступень «затравки».**
task.md задаёт лестницу из трёх ступеней, а не двух:
...
| **Что сделано (пилот)** | **20** | **7** | **1** | **7** |

Пилот — это не затравка, а вчетверо меньше неё, зато с большим числом моделей
и одним роллаутом вместо двадцати пяти. Отсюда два вопроса, которые надо задать
пользователю **до** авторинга, потому что они меняют объём работы в разы:

1. Идём ли мы сразу на полный набор (120–180 сцен × 12–13 осей) или сначала
   закрываем затравку (60–80 сцен × 6 осей × 25 роллаутов × 2–3 модели)?
2. Что делать с числом роллаутов. Затравка требует 25 на вариант, полный набор
   — «меньше роллаутов»; у нас сейчас 1, и «меньше единицы» не бывает. Пилотное
   решение «повторы не добираем» принималось для проверки идеи, а не для
   финального замера, и на полном наборе его надо пересмотреть: при n=1 часть
   Δlang — измеренный шум сэмплирования (совпало 9 повторов из 12).


Это следующая крупная фаза. Ниже — что уже есть, что осталось и где мины.
Цель из `task.md` («Потом полный набор»): **120–180 задач × 12–13 вариантов**,
то есть порядка 1500–2300 промптов против 127 в пилоте.

**Сначала о единице счёта — здесь легко ошибиться.** «Задача» в task.md — это
**сцена** (`task × init state`), а не определение задачи. Доказательства два:
затравка требует «60–80 задач» из сьютов Spatial/Object/Goal, где определений
всего 30, столько задач оттуда физически не набрать; и таблица «Квоты v0»
подписана «для 20 задач», а пилот отобрал ровно 20 **сцен**. Не спутайте: пилот
— это 20 сцен всего из 10 определений задач.

> ⚠ **Но не все они годятся.** 270 сцен из 412 — сьют `libero_90`, которого
> task.md не называет и на котором **не дообучен ни один наш чекпойнт**
> (OpenVLA-OFT: `...-libero-spatial-object-goal-10`; все опубликованные базовые
> линии — по Spatial/Object/Goal, у π0.5 плюс Long). SR там был бы около нуля
> по причине, не связанной с языком. Разбор и таблица пулов —
> [`docs/FULL_SET_PLAN.md`](docs/FULL_SET_PLAN.md), раздел «Те ли это сцены».
> **Решить, какие сьюты входят, надо до разметки видимости**, иначе две трети
> ручного труда уйдёт в мусор.

**Сцены-кандидаты уже собраны — заново не собирать.** `data/full_set/` содержит
**412 сцен**: 390 LIBERO (130 определений × 3 init state, все пять сьютов) и
22 SimplerEnv (4 bridge-задачи). Отрендерены 802 кадра (50 МБ).

Это **пул кандидатов, а не готовый набор**: из него надо отобрать 120–180 сцен
под квоты, как в пилоте отбирали 20 из 117.

Запаса хватает, и вот почему. В пилотном инвентаре `settle_steps` не
выставлялся вовсе, поэтому кадры снимались до того, как предметы улягутся, и
часть сцен отсеивалась именно из-за «летающих» объектов. В полном наборе
`settle_steps: 40` — всё оседает до съёмки, и эта категория брака уходит.
Осторожно с пилотной статистикой годности: 41 из 117 — это не доля годных, а
артефакт того, что 52 сцены вообще не пересматривались; среди реально
просмотренных годными были **41 из 65, то есть 63%**. При такой доле 412
кандидатов дают запас с большим избытком на цель 120–180.

**Отдельно про разнообразие.** 390 LIBERO-сцен — это 130 определений по 3 init
state. Брать 120–180 сцен можно двумя способами: вширь (по одной сцене со
многих определений) или вглубь (по 3 сцены с сорока). Вширь честнее для
бенчмарка — иначе половина набора окажется вариациями одной и той же задачи, а
пилот именно этим и грешил (20 сцен на 10 определений). Это решение принимает
пользователь.

**Потолок SimplerEnv.** Bridge-задач всего 4, сцен 22, и больше взять неоткуда.
Асимметрия сеток моделей и сред уже зафиксирована в task.md как известное
ограничение — но помните, что нарастить bridge-часть нельзя.

| Сьют | Сцен | Уникальных задач |
| --- | --- | --- |
| libero_90 | 270 | 90 |
| libero_spatial / object / goal / 10 | по 30 | по 10 |
| simpler (bridge) | 22 | 4 |

**Что в этих записях уже заполнено** (проверено 08.08.2026, не на глаз):

- `success_predicates` — **412 из 412**, вытащены из BDDL. Готовый сигнал под
  квоты: `in` 189, `on` 183, `close`/`open` 54, `turnon`/`turnoff` 27,
  `src_on_target` 22 (SimplerEnv).
- картинки — 802 файла, битых путей нет; объекты — 1988, у всех есть
  `sim_handle` и поза.
- `candidate_slots` — только у **22 SimplerEnv-сцен**; у всех 390 LIBERO
  пусто.
- `quota_eligibility` — **пусто у всех 412** (все девять флагов `null`).
- видимость — **0 из 1988** объектов размечено. `usable_for_slava` — `null`
  у всех.

То есть это чистый инвентарь с корректной геометрией и предикатами, а не
полузаполненный набор: заготовок, которые надо «подтверждать», здесь нет
почти нигде.

**Что пусто и составляет саму работу:**

| Поле | Единиц | Чем размечается |
| --- | --- | --- |
| `usable_for_slava` | 412 сцен | `slava-quota-eligibility`, отбор |
| `visible_agentview` / `visible_wrist` | **1988 объектов** | `slava-visibility-review` |
| роли и слоты (`target`/`reference`/`relation`/`forbidden`) | ~150 сцен | `slava-scene-roles` |
| `object_lexicon.csv` | новые объекты сверх нынешних 27 | `slava-object-lexicon` |
| варианты инструкции, 12–13 осей | **~1800–2300 строк** | `slava-instruction-variants` |
| `mt_russian` | те же сцены | `slava-mt-russian` (нужен `DEEPL_API_KEY`) |
| `token_len` | те же строки | `slava-token-len` (отдельный venv) |
| native check | все русские строки | `slava-native-check` |

**Главная опасность — масштаб ручного труда.** В пилоте это было 20 сцен,
140 инструкций и 155 объектов, и всё делалось глазами. Здесь то же самое
умножается на 10–15. Любой шаг, который стоит человеку минуту на единицу,
превращается в десятки часов. Прежде чем начинать любой этап, спросите: он
масштабируется или его надо сначала автоматизировать/сэмплировать?

**Ось, которой нет ни в коде, ни в скилах: `zh-подмножество`.** task.md в
описании полного набора требует «+ zh-подмножество», то есть китайские
инструкции. Сейчас под это нет ничего: в `object_lexicon.csv` нет колонок `*_zh`,
native check умеет только русский, `mt_russian` — отдельное поле под один язык.
Это не забытая мелочь, а отдельная ветка контракта данных; поднимите вопрос с
пользователем до того, как схема фрейма будет зафиксирована на 120–180 сценах,
иначе добавлять язык придётся ретроспективно по всему набору.

**Три ошибки пилота, которые нельзя повторить:**

1. **Четыре оси были написаны и ни разу не запущены.** Во фреймах пилота
   заполнено **11 осей**, а `scripts/export_prompts.py` выгружает **7**:
   шесть «primary» из «Сначала затравка» плюс `mt_russian`. В стол ушли
   `ru_free_order` (20 сцен), `ru_colloquial` (20), `ru_translit` (20),
   `ru_anaphora` (11) — 71 написанная инструкция, по которой нет ни одного
   эпизода. Трёх последних нет даже в `VARIANT_ORDER` генератора отчёта, то
   есть они невидимы и в таблицах.

   На полном наборе осей 12–13, и **список выгрузки надо расширять вместе с
   авторингом**, иначе труд снова уйдёт в стол. Инвариант, который стоит
   проверять скриптом перед каждым сбором: множество осей, заполненных во
   фреймах, совпадает с `PRIMARY_VARIANTS` в `export_prompts.py` и с
   `VARIANT_ORDER` в генераторе отчёта. Расхождение — не мелочь, а потерянная
   работа.
2. **`ru_case_swap` — зонд, а не задача.** Предикат среды намеренно не
   переворачивается, поэтому успех считается из финальных поз против
   *перевёрнутой* инструкции (`auto_label._swapped_success`,
   `success_source`). Применим только к сценам, где есть два переставляемых
   предмета (в пилоте 8 из 20). Ось `axis_na` обязана это фиксировать.
3. **Горизонт эпизода брали свой, а не авторский.** Теперь `max_steps_for()`
   выдаёт лимит по сьюту, и авторские значения для новых сьютов уже
   проставлены (`libero_90` — 400 шагов, `libero_10` — 520). Проверьте это
   до сбора, а не после: в пилоте несоответствие обнаружилось задним числом
   и спаслось только тем, что ни один успех не пришёлся на лишние шаги.

### Что требует пользователя (не делать самостоятельно)

0. **Бюджет ручной проверки на полном наборе — решение пользователя, и его
   надо принять ДО начала авторинга, а не после.** В пилоте видимость,
   роли, native check и квоты проверялись глазами по каждой сцене; на 412
   сценах и 1988 объектах это уже расписание, а не деталь. Варианты:
   сплошная проверка, стратифицированная выборка (скажем, 10%) с
   автоматическим прокси на остальное, или проверка только тех осей, что
   идут в метрики. Спросите формой с вариантами — молча выбирать нельзя.
