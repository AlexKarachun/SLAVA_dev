...
<section>
  ...
  <p class="muted">Метки исходов проставляет скрипт, и до проверки они были выводом непроверенной
  программы, а не данными. Поэтому 100 эпизодов были отсмотрены вручную по видеозаписям:
  выборка стратифицирована по моделям, вариантам инструкции и типам ошибки, вердикт человека
  считается истиной, а каждое расхождение разбиралось как возможная ошибка авторазметчика.</p>
  ...
  <p class="muted">Разница между строками — главный вывод проверки: <b>факт успеха
  определяется надёжно, а классификация причины провала ошибается примерно в каждом пятом
  случае.</b> Поэтому SR и Δlang в разделах ниже опираются на надёжную величину, а разбор по
  типам ошибок читается как указание направления, а не как точное измерение.</p>

  <h3>Что проверка изменила в коде разметки</h3>
  <p class="muted">Разбор расхождений дал три исправления, каждое подсказано конкретными
  замечаниями проверяющего. Согласие пересчитано на одной и той же выборке из 100 эпизодов
  после каждого исправления:</p>
  ...
  <p class="muted">Последняя строка снижает согласие по метке на единицу и одновременно
  поднимает согласие по факту успеха с 97 до 99 из 100 — именно этого исправления проверка и
  требовала. Все три пересчитаны из сохранённых логов, без повторных запусков на GPU.</p>

  <p class="muted">Оговорка о том, как читать эти числа. Вердикты снимались до исправлений,
  поэтому 80 / 100 — оценка нынешнего кода на разметке, собранной для прежнего. Кроме того,
  57 эпизодов из 100 проверяющий разобрал явно, а остальные 43 оставил без изменений, что по
  его же указанию считается согласием; при более строгом счёте (только явно проверенные)
  согласие по метке 37 / 57.</p>
</section>
