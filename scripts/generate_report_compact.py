...
<p class=ci>Огрехи в <code>mt_russian</code> — не опечатки, а выход системы
перевода: правило <code>task.md</code> запрещает его редактировать, иначе
контроль перестаёт измерять то, ради чего существует. Все остальные русские
варианты выверены вручную.</p>
<p class=ci><b>Оговорка к этому сравнению.</b> В 2 сценах из 20 сырой перевод
называет не тот объект: <code>plate</code> переведён как «плита» и «противень»,
то есть инструкция требует двигать электроплиту вместо тарелки. Это 8 эпизодов
из 84 с <code>mt_russian</code>, все провальные. Значит SR этого варианта —
нижняя оценка, где смешаны языковой эффект и ошибки перевода, а разрыв с
<code>ru_literal</code> нельзя читать как чистую разницу «сырой MT против
выверенного русского».</p>
{mt_html}
