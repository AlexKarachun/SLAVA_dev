...
LOCAL_ANNOTATIONS="${PROJECT_ROOT}/rollouts/rollout_annotations.jsonl"
LOCAL_EPISODES="${PROJECT_ROOT}/rollouts/episodes"
TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT

echo "== аннотации"
# shellcheck disable=SC2086
scp ${REMOTE/ /:}"${REMOTE_ROOT}/rollouts/rollout_annotations.jsonl" \
  "${TMP}/remote.jsonl" >/dev/null 2>&1 || {
    ...
    # shellcheck disable=SC2086
    ssh ${REMOTE} "cat ${REMOTE_ROOT}/rollouts/rollout_annotations.jsonl" > "${TMP}/remote.jsonl"
  }
...
# shellcheck disable=SC2086
ssh ${REMOTE} "cd ${REMOTE_ROOT}/rollouts/episodes && tar -czf - -T -" \
