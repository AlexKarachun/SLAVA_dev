...
- **Покрытие полное**: 536 эпизодов собрано, все 536 в метриках. 36 эпизодов
  `ru_case_swap` пересчитаны офлайн 08.08.2026 по исправленному критерию (успех
  считается из финальных поз, а не из предиката среды) и возвращены в метрики.
  Каждая существующая клетка «модель × среда» заполнена целиком. Что и почему когда-либо
