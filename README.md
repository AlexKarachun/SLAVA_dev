...
- **pi0 / pi0.5 / SmolVLA исключены из агрегатов отчёта**: их эпизоды
  собраны по обе стороны от правки общего `lerobot_server.py`, а отчёт
  намеренно не смешивает две конфигурации инференса в одно число (см.
  `annotate_provenance()`). Чтобы получить по ним валидные цифры, эти три
  модели нужно переснять целиком. Сейчас в метриках отчёта — 183 эпизода
  из 551 собранных.
