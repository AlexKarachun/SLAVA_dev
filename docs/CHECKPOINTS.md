...
Часть настроек в `scripts/model_servers/lerobot_server.py` выбирается **по
железу**, а не зашита намертво, поэтому один и тот же код на разных картах
считает по-разному. Что именно чем собрано — в
[`data/collection_runs.json`](../data/collection_runs.json), одна запись на
сеанс сбора.

Правило простое. До Ampere (compute capability < 8.0) bf16-ядер нет: дефолтный
для pi0.5 `bfloat16` там даёт не замедление, а падение в patch-embedding Conv2d
у SigLIP — поэтому на Volta принудительно `float32` и отключённый cuDNN. С
Ampere и новее берётся собственный `dtype` чекпойнта: это и то, на чём его
запускали авторы, и вдвое меньше памяти под веса, что позволяет держать
несколько шардов model-сервера на одной карте.

`SLAVA_COMPAT_V100=1` форсирует настройки Volta на любой карте — для
воспроизведения опубликованных чисел, а не для сбора новых.

Что из этого следует для чтения таблиц: сравнение внутри модели (между
вариантами инструкции, то есть Δlang) всегда идёт внутри одного сеанса сбора и
разной точностью не затронуто. А вот абсолютный SR двух моделей, собранных при
разных настройках, без оговорки сравнивать нельзя.

## Что из этого уже собрано

Пересчитывается из `rollouts/final/pilot_v0/rollout_annotations.jsonl`.
...
| pi0 | 99 / 99 ✓ | 28 / 28 ✓ |
| pi0.5 | 99 / 99 ✓ | 28 / 28 ✓ |
| SmolVLA | 99 / 99 ✓ | клетки нет |

Итого 536 эпизодов собрано, **все 536 идут в метрики**: 36 эпизодов `ru_case_swap`, снятых 07.08.2026 после смены критерия успеха, пересчитаны офлайн 08.08.2026 из сохранённых `object_poses` (`scripts/relabel_rollouts.py --include-case-swap`) и возвращены в набор. Эпизоды
pi0/pi0.5/SmolVLA пересняты 06.08.2026 текущим кодом (сброс очереди действий
между эпизодами, исправленные карты камер, проприоцепция в оси-угле,
`transformers` внутри пина lerobot); прежние лежат в
`rollouts/archive/lerobot_pre_reset_fix/episodes/`.

`data/rollout_provenance.json` держит две записи. Первая — SmolVLA × SimplerEnv:
это не отложенный пересбор, а объявление, что пара не определена (см. выше),
таких эпизодов в текущем наборе аннотаций нет вовсе. Вторая — 36 эпизодов
`ru_case_swap`, снятых по прежнему критерию успеха; они остаются в файле
аннотаций как есть, но в метрики не идут.
