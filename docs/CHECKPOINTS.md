...
Итого 536 эпизодов, все идут в метрики: исключений не осталось. Эпизоды
...
`data/rollout_provenance.json` держит одну запись — SmolVLA × SimplerEnv. Это не
отложенный пересбор, а объявление, что пара не определена (см. выше): таких
эпизодов в текущем наборе аннотаций нет вовсе.
