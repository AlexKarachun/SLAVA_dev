...
## Читая чужой код: не спешить с выводом «у авторов ошибка» (08.08.2026)

Правило появилось после конкретного промаха, и его стоит помнить при любой
сверке с апстримом.

Разбирая GreenVLA, я прочитала один класс (`BridgeOutputsTransform`), увидела,
что он не делает того, что обещает докстрока авторского примера, и записала в
три документа вывод «комментарий авторов противоречит их коду». Вывод оказался
неверным: `output_transforms` — это composed-цепочка, денормализация лежит в
`UnnormalizeTorch` уровнем выше, а `state` читается только в ветке
`actions_type == "absolute"`, которой bridge не пользуется. Хуже того, в этом же
скиле с 05.08.2026 уже стоял правильный разбор ровно этого места — я его
переоткрыла и переврала.

Что из этого следует:

1. **Асимметрия доказательности.** У авторов их код даёт заявленные числа, у нас
   нет. При таком раскладе расхождение — почти всегда наш дефект, и априор надо
   держать соответствующий. «Мы нашли противоречие» — это повод искать ошибку у
   себя, а не повод обвинять апстрим.
2. **Часть цепочки — не цепочка.** Прежде чем утверждать, что шаг отсутствует,
   найти сборщик (`get_*_transforms`, `compose`, фабрику) и прочитать порядок
   целиком. Отдельный класс почти никогда не является всем преобразованием.
3. **Сначала искать в скилах.** Если место уже разбиралось, там лежит вывод с
   доказательством. Переоткрытие без чтения — это не независимая проверка, а
   потерянная работа плюс риск заменить верный вывод неверным.
4. **Формулировать по силе доказательства.** «Прочитан класс X, в нём шага нет»
   — честно. «У авторов ошибка» — заявка совсем другого веса, и она требует
   прочитанной целиком цепочки и объяснения, почему у них при этом всё работает.
...
## Кубик, вылетающий из гриппера — артефакт контактов симулятора (07.08.2026)

Найдено пользователем при ручной валидации. В эпизоде
`simpler__widowx_stack_cube__episode001__...__greenvla_r1_bridge` зелёный кубик
после касания улетел на 65 см и упал со стола на пол.

Разбор по логу шагов однозначный:

```
шаг 17-22  команда гриппера -0.97 (сжать), состояние 0.45 → 0.95   — смыкается на кубике
шаг 23     команда  +0.98 (разжать), состояние 0.94 → 0.35         — резкий отпуск
шаг 22-24  кубик:  Δ24 см → Δ32 см → Δ63 см, z с 0.885 до 0.019    — вылет и падение на пол
```

**Команды позиции в этот момент — 0.012-0.027**, то есть сантиметр-два за шаг:
ничем таким кубик отбросить нельзя. Значит это не наша шкала действия и не
единицы измерения — это контактный решатель: пока пальцы сжаты, накапливается
взаимопроникновение с мелким объектом, и при размыкании оно разряжается
импульсом. Классический артефакт жёстких контактов, тот же в MuJoCo и SAPIEN.

Наш вклад — только провоцирующий: политика жёстко переключает канал гриппера
с −1 на +1 за один шаг. Смягчать это на нашей стороне нельзя: команда гриппера
идёт от модели как есть, и «сглаживание» ради красивой физики было бы
вмешательством в то, что мы измеряем.

**Насколько часто.** Скрин по всем 536 эпизодам (максимальный сдвиг объекта за
один шаг): **2 эпизода**, где сдвиг больше 15 см — 0.63 м в этом случае и 0.20 м
в одном LIBERO-эпизоде. То есть 0.4% датасета, на метрики не влияет.
Воспроизводится сравнением соседних `object_poses` в `steps.jsonl`.

**Как размечать такие эпизоды вручную:** провал с меткой
`physical_execution_error` — намерение было верным, объект выбран правильно, не
получилось физически, — и заметка про вылет. Не `unclear`: причина как раз
понятна.

## Метки провалов человеческим языком (для ручной валидации)

`task.md` задаёт метки формально, но человеку, который смотрит запись, нужна
формулировка, по которой можно решать быстро. Эти определения согласованы с
пользователем 07.08.2026 и использованы при валидации первых 100 роллаутов —
держать их согласованными с `task.md`, а не переписывать заново:

- **Не тот target** — робот первым тронул не тот объект, который назван целью.
  Ошибка на самом первом шаге: модель не поняла, о чём речь.
- **Не тот reference** — цель взята правильно, но относительно чего её ставят
  выбрано неверно: «положи морковь на тарелку», а он несёт её к миске.
- **Не то отношение** — и цель, и ориентир правильные, а отношение между ними
  нарушено: «слева» вместо «справа», рядом вместо «на».
- **Нарушил запрет** — робот тронул объект, явно названный в инструкции
  неправильным («не верхний ящик, а средний» — полез в верхний). Только для
  вариантов с отрицанием (см. правило про ось отрицания ниже).
- **Физика** — понял всё правильно и делал что надо, но не вышло руками: не
  схватил, уронил, не дотащил.
- **Не двигался** — осмысленного действия не было: замер, дёргался на месте,
  доиграл до лимита шагов.
- **Непонятно** — по записи нельзя уверенно сказать, что произошло. Ставится
  смело: честнее домысла, и `task.md` предусматривает метку именно для этого.

Практика первых 100: чаще всего человек расходится с автоматом там, где
автомат ставит `relation_binding_error`, а человек видит «физику» (не смог
взять) — 5 случаев из 24 расхождений. Это подсказка, где правило слабее всего.

## Контракт авторазметки изменён 07.08.2026 — два правила, которые нельзя вернуть назад

Оба дефекта нашёл пользователь при обязательной ручной валидации первых 100
роллаутов; оба меняли смысл колонок, а не только их значения.

**1. `negation_error` — только на оси отрицания.** Слоты `forbidden_objects`
заполнены у всех вариантов сцены, поэтому метка срабатывала везде, где рука
задевала этот объект. Из 21 метки в пилоте на `ru_negation` приходилось 4;
17 остальных (en_canonical, mt_russian, ru_literal, code_switch) перемечены в
`target_grounding_error`. Запрет существует только там, где инструкция его
произносит. Сырой сигнал `forbidden_object_touched` пишется по-прежнему для всех
вариантов — по нему можно пересчитать более строгое правило без пересбора.

**2. `ru_case_swap` — успех считается по перевёрнутой инструкции.** Предикат
среды намеренно не переворачивается вместе с текстом, поэтому `env_success`
отвечал на вопрос «сделал ли робот ИСХОДНОЕ задание», и высокий SR означал «не
заметил перестановку» — обратное тому, ради чего зонд существует. Теперь
`auto_label._swapped_success` проверяет по финальным позам, оказался ли нужный
объект на другом (все 8 промптов используют отношение `on`; пороги 6 см по
горизонтали и 1-12 см по высоте покрывают и кубик, и тарелку). Нет поз — «не
знаю», откат на `env_success`, а не выдуманный провал.

**3. Нельзя нарушить отношение объектом, который не поднимали.** Если цель ни
разу не оторвалась от поверхности выше 2 см, эпизод получает
`physical_execution_error`, а не `relation_binding_error` (та ставилась по
остаточному принципу). Перемечено 56 эпизодов. Подъём считается из
`object_poses` в `steps.jsonl`; `None` (поз нет) сохраняет прежнее поведение.

Суммарно три правки подняли согласие авторазметки с человеком по типу ошибки
с 76 до 81 из 100 на выборке ручной валидации (после возврата `ru_case_swap`
в метрики — 80/100, зато факт успеха вырос с 97 до 99/100).

**Числа считать только скриптом, не по памяти.** Ряд «76 → 87 → 92», стоявший
здесь до 08.08.2026, не воспроизводился ни одним способом подсчёта из
сохранённых артефактов. Воспроизводимый способ: снимки разметки лежат в
`rollouts/archive/annotation_snapshots/`, вердикты — в
`data/label_review/verdicts_2026-08-07.json`, сверка — `scripts/apply_label_review.py`
(43 карточки без правок считаются согласием по прямому указанию пользователя).

**Новое обязательное поле строки: `success_source`** (`env` /
`swapped_predicate`). Без него единица в колонке `success` не читается: непонятно,
предикатом среды она получена или нашей проверкой.

Уже собранные 36 эпизодов `ru_case_swap` намеренно не переписаны, а помечены
устаревшими в `data/rollout_provenance.json` (решение пользователя: результаты
не подменяем задним числом). В метриках 500 эпизодов из 536.

**Следствие для будущих прогонов:** после пересбора `ru_case_swap` исключение из
провенанса снимается, и SR этого варианта начинает означать долю моделей,
которые перестановку заметили и выполнили. Сравнивать это число со старым
нельзя — они про разное.
