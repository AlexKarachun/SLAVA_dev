...
## Горизонт открытого цикла: 50 против авторских 10 (найдено 08.08.2026)

`config.json` у `lerobot/pi0_libero_finetuned` и `lerobot/pi05_libero_finetuned`
объявляет **`n_action_steps = 50`** (`chunk_size` тоже 50). Мы брали это значение
как есть, то есть проигрывали 50 предсказанных действий подряд, ни разу не
взглянув на сцену.

Авторский eval так не делает. Команда, которой lerobot получил свои 97.5%
(`docs/source/libero`), содержит `--policy.n_action_steps=10` с пояснением
*«matching the original OpenPI implementation»*. За эпизод в 300 шагов у них 30
перепланирований, у нас было 6.

Почему это важно: политика с чанкингом обучена корректировать себя, регулярно
глядя на сцену; проигрывая полсотни шагов вслепую, робот едет по устаревшему
плану. Совпадает с почерком π0 в ручной валидации — «плавно перевернулся»
(7 заметок из 13), «улетел за пределы сцены».

Исправлено в `lerobot_server.py`: горизонт принудительно ставится в 10,
переопределяется `SLAVA_N_ACTION_STEPS`. **SmolVLA не затронута** — её чекпойнт
объявляет `n_action_steps=1` и перепланирует каждый шаг; поэтому и почерк отказа
у неё другой («тянется не к тому объекту»).

**Проверить можно только на GPU** — это первый пункт следующей сессии. Полная
сверка с авторским eval — `docs/EVAL_PARITY.md`.

## Шкала действия на SimplerEnv: команда в 20-60 раз больше возможной (08.08.2026)

Отдельный дефект и отдельная среда — LIBERO он не касается.
...
стоят ±1.0 как заглушка именно потому, что нормировка выключена), то есть ждёт
позиционную дельту **в метрах**.

Среднее |a| по позиции, посчитано по нашим `steps.jsonl`, только SimplerEnv —
один робот, один контроллер:

| Модель | позиция (м/шаг) |
| --- | --- |
| GreenVLA-R0/R1/R2 | 0.003-0.008 — физически осмысленно |
| **π0** | **0.104 / 0.095 / 0.333** |
| **π0.5** | **0.175 / 0.238 / 0.324** |

10-33 см за шаг — команда, которую этот робот исполнить не может. Это
измерение превращает известную оговорку в дефект: `pi0_base` и `pi05_base`
**не везут статистик нормализации вообще** (см. `docs/CHECKPOINTS.md`), поэтому
их выход на bridge ничем не отшкалирован. Первый кандидат по SimplerEnv-половине
этого сервера, задача T12.5. LIBERO-половина этим не объясняется — там
чекпойнты дообученные и со своими статистиками, и подозреваемый другой (горизонт
открытого цикла выше).
