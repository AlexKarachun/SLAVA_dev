...
## Сырой MT иногда подменяет сам объект — это не опечатка, а конфаунд

Найдено 07.08.2026 при ручной валидации, промпт 88. Английское
`push the plate to the front of the stove` DeepL перевёл как
**«подвиньте плиту к передней части кухонной плиты»**: `plate` стало «плитой»,
то есть целью названа сама электроплита, к которой её же и надо подвинуть.
В соседнем init-состоянии тот же английский текст переведён иначе —
«подвиньте противень к передней части духовки»: снова не тарелка.

Скрин по всем 20 сценам пилота (имя цели из лексикона против текста
`mt_russian`) даёт **2 сцены из 20**, где MT называет не тот объект, — это
**8 эпизодов из 84** с вариантом `mt_russian`, и все 8 провальные.

Почему это важнее грамматических огрехов: неуклюжая формулировка всё ещё
описывает нужную задачу, а подменённый объект — уже **другая задача**. Модель,
провалившая такой эпизод, провалила неверную инструкцию, а не русскую. Значит
SR по `mt_russian` — нижняя оценка, загрязнённая ошибками перевода, и разрыв
`mt_russian` против `ru_literal` нельзя целиком трактовать как «сырой MT хуже
человеческого русского по стилю».

**Править всё равно нельзя.** Правило `task.md` — «не редактировать, не
улучшать, не нормализовать»; вариант существует, чтобы показать, что бывает с
инструкцией, когда её просто прогнали через переводчик, и подмена объекта — как
раз самый убедительный пример этого. Правильная реакция: посчитать, сколько
